...
def gerar_vetor_dac_parametrizado(frequencia_onda, amostras_por_ciclo, dac_bits, amplitude_normalizada, porcentagem_70, porcentagem_90):
    ...
    if porcentagem_70 < 0:
        raise ValueError("A porcentagem da frequência de 70º ordem deve ser positiva.")
    if porcentagem_90 < 0:
        raise ValueError("A porcentagem da frequência de 90º ordem deve ser positiva.")
    ...
    amplitude_dac_70 = (porcentagem_70/100) * amplitude_dac
    amplitude_dac_90 = (porcentagem_90/100) * amplitude_dac
    dac_valores = [
        int(round(max(0, min(offset + 
                             amplitude_dac * math.sin(2* math.pi * i / amostras_por_ciclo)+
                             amplitude_dac_70 * math.sin(70 * 2 * math.pi * i / amostras_por_ciclo)+
                             amplitude_dac_90 * math.sin(90 * 2 * math.pi * i / amostras_por_ciclo),
                             max_dac_val))))
...
def salvar_vetor_em_arquivo_c(filename, vetor_dac, freq_onda, num_amostras, res_dac, amp_norm, porc_amplitude_70, porc_amplitude_90, freq_amostragem, prd_timer_val):
    ...
    with open(filename, "w") as f_c:
        ...
        f_c.write(f" * Porcentagem 70º: {porc_amplitude_70}\n")
        f_c.write(f" * Porcentagem 90º: {porc_amplitude_90}\n")
...
def senoide():
    ...
    try:
        ...
        porc_amplitude_70 = float(input("Porcentagem da 70ª Harmônica: "))
        porc_amplitude_90 = float(input("Porcentagem da 90ª Harmônica: "))
        vetor_dac, freq_amostragem = gerar_vetor_dac_parametrizado(freq_onda, num_amostras, res_dac, amplitude_norm, porc_amplitude_70, porc_amplitude_90)
        ...
        print(f"Porcentagem 70º: {porc_amplitude_70}")
        print(f"Porcentagem 90º: {porc_amplitude_90}")
        ...
        salvar_vetor_em_arquivo_c("dac_buffer_values.c", vetor_dac, freq_onda, num_amostras, res_dac, amplitude_norm, porc_amplitude_70, porc_amplitude_90, freq_amostragem, prd_timer)
...
def comparar_fft_dac_adc(dac_valores, adc_valores, freq_amostragem_dac, freq_amostragem_adc):
    ...
    # === Gráfico 3: Comparação FFT DAC vs ADC ===
    plt.figure(figsize=(8, 4))
    ...
    try:
        with serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=2) as ser:
            ...
            while True:
                ...
                print("5. Comparar FFT DAC vs ADC")
                print("0. Sair")
                opcao = input("Escolha: ")
